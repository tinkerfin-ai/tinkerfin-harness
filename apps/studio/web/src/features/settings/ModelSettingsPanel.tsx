...
export function ModelSettingsPanel({ onChanged, onToast }: { onChanged?: () => void; onToast: (kind: ToastKind, message: string) => void }) {
  ...
  const connection = state.connections.find(item => item.connection_id === selected) ?? state.connections[0]
  const models = state.models.filter(model => model.connection_id === connection?.connection_id)
  const providers = state.connections.filter(item => (item.display_name + ' ' + state.models.filter(model => model.connection_id === item.connection_id).map(model => model.display_name + ' ' + model.model_name).join(' ')).toLowerCase().includes(search.toLowerCase()))
          ...
          : view === 'discovery' && connection ? <ModelDiscoveryPanel connection={connection} models={models} saving={state.saving} onAdd={state.addModels} onManual={() => editModel(newModel(connection.connection_id))} onCancel={close} />
            : <>
              {state.loadFailed ? <div role="status"><p>{t('模型加载失败，请先重试')}</p><Button type="button" onClick={state.reload}>{t('重新加载模型')}</Button></div> : state.loading ? <p role="status">{t('正在加载模型配置')}</p> : state.connections.length === 0 ? <div className="settings-models__empty"><Cpu size={28} /><h4>{t('连接你的第一个模型服务')}</h4><p>{t('添加云端提供方，或连接本地 Ollama')}</p><Button type="button" onClick={() => setView('new')}>{t('添加提供方')}</Button></div> : <>
                ...
                <ModelProviderSplit sidebar={<nav className="settings-models__providers" aria-label={t('提供方')}>
                  ...
                  {providers.map(item => {
                    ...
                    return <button type="button" key={item.connection_id} title={item.display_name} className={`settings-models__provider${connection?.connection_id === item.connection_id ? ' is-selected' : ''}`} aria-current={connection?.connection_id === item.connection_id ? 'true' : undefined} onClick={() => setSelected(item.connection_id)}><Icon size={18} /><span><strong>{item.display_name}</strong><small>{t('{count} 个模型', { count: state.models.filter(model => model.connection_id === item.connection_id).length })}</small></span></button>
                  })}
                  {providers.length === 0 && <p className="settings-models__muted">{t('没有匹配的提供方或模型')}</p>}
