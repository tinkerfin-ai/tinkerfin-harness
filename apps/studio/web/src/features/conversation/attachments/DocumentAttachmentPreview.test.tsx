...
describe('文档预览', () => {
  it('经原件鉴权接口读取 Markdown，安全展示并关闭恢复焦点', async () => {
    ...
    const info = within(dialog).getByRole('button', { name: '文件信息' })
    await user.click(info)
    expect(within(dialog).getByRole('complementary', { name: '文件信息' })).toBeVisible()
    await user.keyboard('{Escape}')
    expect(within(dialog).queryByRole('complementary', { name: '文件信息' })).not.toBeInTheDocument()
