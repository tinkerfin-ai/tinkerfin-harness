...
test('真实跨域表单直传保留文件与取消，业务凭据仅发送给后端', { timeout: 30_000 }, async () => {
  ...
  let vite, browser
  try {
    vite = await createServer({ root: webRoot, configFile: false, cacheDir: cache,
      appType: 'custom', server: { host: '127.0.0.1', port: 0, hmr: false, ws: false, watch: null, proxy: { '/api': { target: `http://127.0.0.1:${upstream.address().port}` } } },
    })
    ...
    held?.destroy()
    for (const server of [upstream, objectServer]) {
