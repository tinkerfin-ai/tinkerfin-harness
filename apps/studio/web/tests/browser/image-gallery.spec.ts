...
test('图片失败态、恢复与多图连续键盘浏览保持无边框布局', async ({ page }, testInfo) => {
  ...
  await page.route('**/api/**', async route => {
    ...
    else if (url.pathname === '/api/models') data = { items: [{ modelId: history.lastModel, displayName: '测试模型', imageSupport: 'supported', reasoningEnabled: true, isDefault: true }], defaultModelId: history.lastModel }
