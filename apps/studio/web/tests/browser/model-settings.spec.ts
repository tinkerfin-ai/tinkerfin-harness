...
async function setup(page: Page, crowded = false) {
  ...
  await page.route('**/api/**', async route => {
    ...
    else if (path === '/api/models') data = {items:models.map(model=>({modelId:model.model_id,displayName:model.display_name,reasoningEnabled:model.reasoning_enabled,imageSupport:model.image_support,isDefault:model.is_default})),defaultModelId:'pro'}
