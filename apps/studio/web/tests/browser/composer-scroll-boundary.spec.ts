...
for (const theme of ['light','dark']) test(`输入框及菜单滚动不移动会话 ${theme}`, async ({page}) => {
  ...
  await page.route('**/api/**',async route => {
    ...
    else if (path === '/api/models') data = {defaultModelId:'model-0',items:Array.from({length:35},(_,i)=>({modelId:`model-${i}`,displayName:`Model ${i}`,connectionId: 'test-provider', connectionDisplayName: '测试提供方', reasoningEnabled:false,isDefault:i===0}))}
